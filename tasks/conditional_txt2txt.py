...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,
                 folder: str):
        """
        conditional_txt2txt task dataset.
        ...
        cond_files = [*path.glob('**/*.ctx')]
        text_files = {text_file.stem: text_file for text_file in text_files}
        cond_files = {cond_file.stem: cond_file for cond_file in cond_files}
        keys = (text_files.keys() & cond_files.keys())

        self.keys = list(keys)
        self.text_files = {k: v for k, v in text_files.items() if k in keys}
        self.cond_files = {k: v for k, v in cond_files.items() if k in keys}
    ...
    def __getitem__(self, ind):
        ...
        cond_file = self.cond_files[key]
        try:
            descriptions = text_file.read_text().split('\n')
            contexts = cond_file.read_text().split('\n')
        except UnicodeDecodeError:
            return self.skip_sample(ind)
        descriptions = list(filter(lambda t: len(t) > 0, descriptions))
        contexts = list(filter(lambda t: len(t) > 0, contexts))
        try:
            idx = choice(range(len(descriptions)))
...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if self.web_dataset:
            ...
            myctx, mycap = (self.wds_keys.split(',')[0], self.wds_keys.split(',')[1])
            text_mapping = {
                            myctx: self.decode,
                            mycap: self.decode,
                        }
